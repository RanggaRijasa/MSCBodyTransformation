...
phase07_temp_dir="$(mktemp -d "$web_root/.phase07.XXXXXX")"
status_file="$phase07_temp_dir/supabase.env"
trap 'rm -f "$status_file"; rmdir "$phase07_temp_dir"' EXIT
...
DO_NOT_TRACK=1 supabase test db --local \
  MSCWeb/supabase/tests/database/phase07_participant_experience.test.sql \
...
DB_URL="$DB_URL" \
AUTH_FLOW_COOKIE_SECRET="phase07-local-cookie-secret-2026-only" \
APP_ENVIRONMENT="phase07-local" \
