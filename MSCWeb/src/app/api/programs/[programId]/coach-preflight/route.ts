...
import { resolveCoachForProgramOperation } from "@/application/programs/program-enrollment-operations";
...
export async function POST(
  ...
  const result = await resolveCoachForProgramOperation(
    (await params).programId,
    body.coachQrPayload,
  );
  if (!result.isSuccess) {
    const status =
