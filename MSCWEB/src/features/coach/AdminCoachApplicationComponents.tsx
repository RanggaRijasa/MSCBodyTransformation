...
function ApplicationDetail({ item }: { item: CoachApplicationQueueItem }) {
  ...
  const [mode, setMode] = useState<'idle' | 'approve' | 'correction' | 'reject'>('idle');
  ...
  const requestCorrection = async () => { if (!order) return; await decision.mutateAsync({ kind: 'correction', order, reason }); setMode('idle'); };
  const decisionForm = mode === 'approve'
    ? <View style={styles.stack}><InlineMessage title="Aktivasi atomik" message="Satu tindakan menyetujui pembayaran, mengaktifkan Coach selama tiga bulan, membuat QR, ledger, dan audit." tone="warning" /><Field label="Referensi rekonsiliasi" value={reference} onChangeText={setReference} /><Check label="Jumlah dan tujuan pembayaran cocok" checked={matches} onPress={() => setMatches((value) => !value)} /><View style={styles.actions}><Button label="Batal" tone="secondary" onPress={() => setMode('idle')} /><Button label="Setujui dan aktifkan Coach" disabled={reference.trim().length < 4 || !matches} loading={decision.isPending} onPress={() => void approve()} /></View></View>
    : mode === 'correction' || mode === 'reject'
      ? <View style={styles.stack}><InlineMessage title={mode === 'correction' ? 'Perbaikan tidak menutup pengajuan' : 'Penolakan bersifat final'} message={mode === 'correction' ? 'Aplikasi dan pesanan yang sama tetap digunakan untuk percobaan bukti berikutnya.' : 'Aplikasi dan pesanan ini ditutup. Pengajuan berikutnya membuat riwayat baru.'} tone={mode === 'correction' ? 'warning' : 'destructive'} /><Field label={mode === 'correction' ? 'Instruksi perbaikan' : 'Alasan penolakan'} value={reason} onChangeText={setReason} multiline /><View style={styles.actions}><Button label="Batal" tone="secondary" onPress={() => setMode('idle')} /><Button label={mode === 'correction' ? 'Minta perbaikan bukti' : 'Tolak pengajuan'} tone={mode === 'correction' ? 'secondary' : 'destructive'} disabled={reason.trim().length < 5} loading={decision.isPending} onPress={() => void (mode === 'correction' ? requestCorrection() : reject())} /></View></View>
      : <View style={styles.actions}><Button label="Minta perbaikan" tone="secondary" onPress={() => setMode('correction')} /><Button label="Tolak pengajuan" tone="destructive" onPress={() => setMode('reject')} /><Button label="Setujui dan aktifkan Coach" onPress={() => setMode('approve')} /></View>;
  return <View style={styles.detail}><View style={styles.requirements}><Requirement label="Level SC atau lebih tinggi" met={application.member_level_snapshot !== 'member'} /><Requirement label="HOM STS" met={application.has_completed_hom_sts} /><Requirement label="ICT" met={application.has_completed_ict} /><Requirement label="Bukti pembayaran dikirim" met={order?.status === 'under_review' || order?.status === 'approved'} /></View>{order ? <><Text style={[styles.bodyStrong, { color: colors.primaryText }]}>Pembayaran {rupiahFormatter.format(order.amount_minor)} · {order.status}</Text><EvidencePreview order={order} /></> : <InlineMessage title="Pembayaran belum dibuat" message="Peserta perlu menyelesaikan langkah aplikasi dan membuat permintaan pembayaran." tone="warning" />}{decision.error instanceof Error ? <InlineMessage title="Keputusan belum diproses" message={decision.error.message} tone="destructive" /> : null}{order?.status === 'under_review' && application.status === 'submitted' ? decisionForm : null}</View>;
}
