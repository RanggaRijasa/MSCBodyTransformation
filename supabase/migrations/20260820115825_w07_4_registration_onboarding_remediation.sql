...
-- The original local W05 verification had a cancellable-order helper that was
-- never captured by migration authority. Define the reviewed implementation
-- here before it is wrapped by the active-account guard below.
...
begin
  ...
  if target_order.id is null
    or not (
      target_order.owner_user_id = (select auth.uid())
      or private.is_admin()
    ) then
    raise exception 'permission_denied';
  end if;
  ...
  if target_order.status not in (
    'awaiting_evidence', 'correction_required', 'expired'
  ) then
